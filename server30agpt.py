# server30agpt.py — Smart Real-Time GPT Forecast Server (Collector-aligned)
...
# === Local imports ===
from telegram_alert30agpt import (
...
from llm_predict3 import forecast_config_update
from redis_util import get_redis, save_gpt_buy_signal_to_redis_can, get_last_timestamp
...
def get_last_forecast_time(r, stock_code):
    key = f"FORECASTGPT_CAN:{stock_code}"
...
def gpt_tf_cooldown_active(r, stock_code, tf_label):
    ...
    key = f"BUY_SIGNAL_GPT_CAN:{stock_code}"
...
def _process_stock_forecast(stock_cfg, tf_label):
    ...
    try:
        ...
        r.hset(f"FORECASTGPT_CAN:{stock_code}", mapping={"last_updated": now_str})
        ...
        for idx, tf in TF_INDEX_MAP.items():
            ...
            if save_gpt_buy_signal_to_redis_can(r, stock_code, tf_label, payload):
                r.hset(
                    f"BUY_SIGNAL_GPT_CAN:{stock_code}",
                    mapping={
            ...
            if batched_updates:
                try:
                    for stock_code, _ in batched_updates:
                        redis_data = r.hgetall(f"FORECASTGPT_CAN:{stock_code}")
                        if not redis_data:
                            continue
                        live_cfg = {
