# #!/usr/bin/env python3
# # history_fetcher.py — fetch historical candles including 1min, 15min, 1day, and 1month directly from Kite
...
# # ✅ You can uncomment other timeframes later (all will work fine)
# BASE_TIMEFRAMES = {
#     "1min": "minute",
#     "5min": "5minute",
#     "15min": "15minute",
#     "30min": "30minute",
#     "1hour": "60minute",
#     "1day": "day",  # ✅ supported directly by Kite
#     "1month": "month",  # ❌ will be resampled from 1day data (not fetched directly)
# }
...
#     print(
#         f"✅ Resampled {len(df)} completed monthly candles up to {df['Timestamp'].max()}"
#     )
#     return df
...
#     from_date = now - timedelta(days=365 * 5)  # 5 years for daily/monthly
...
#                 # ---- 1month (special case) ----
#                 if tf == "1month":
#                     print(f"🧮 Building 1month candles from 1day for {stock_code}...")

#                     daily_key = f"MARKETDATA:{stock_code}:1day"
#                     daily_data = r.hgetall(daily_key)

#                     if not daily_data:
#                         print(
#                             f"⚠️ No daily data found for {stock_code}, fetching it first..."
#                         )
#                         end_date = now - timedelta(days=1)
#                         daily_rows = _kite_fetch_hist(
#                             kite, token, from_date, end_date, "day"
#                         )
#                         if not daily_rows:
#                             print(
#                                 f"⚠️ No daily data available for {stock_code}. Skipping."
#                             )
#                             continue
#                         df_daily = _standardize_df(pd.DataFrame(daily_rows))
#                         df_daily = calculate_indicators(df_daily)
#                         save_to_redis(r, stock_code, "1day", df_daily)
#                     else:
#                         rows = []
#                         for _, v in daily_data.items():
#                             try:
...
#                     df_month = _resample_daily_to_monthly(df_daily)
#                     if df_month.empty:
#                         print(f"⚠️ No valid monthly data for {stock_code}")
#                         continue
...
#                     )
#                     continue  # skip Kite API fetch for 'month'

#                 # ---- 1day (fetch only completed candles) ----
#                 if tf == "1day":
#                     end_date = now - timedelta(days=1)  # exclude today
#                     rows = _kite_fetch_hist(kite, token, from_date, end_date, interval)
#                 else:
#                     rows = _kite_fetch_hist(kite, token, from_date, now, interval)

#                 if not rows:
#                     print(f"⚠️ No data returned for {stock_code} {tf}")
#                     continue
