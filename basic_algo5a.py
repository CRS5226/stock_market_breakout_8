...
def classify_execution(entry: float, target: float, live: float) -> str:
    """
    Decide how this BUY can be executed.
    """
    if live < entry < target:
        return "STOP"

    if abs(entry - live) / live <= 0.0025 and target > entry:
        return "MARKET"

    if entry < live < target:
        return "WAIT"
...
def basic_forecast_update(
    ...
    EXECUTION-AWARE:
    - STOP / MARKET → BUY allowed
    - WAIT / INVALID → logged, not alerted

    DEBUG:
    - Full trace written via debug_log()
    """
    updated_cfg = dict(stock_cfg)
    now_ts = dt.datetime.now(IST).strftime("%Y-%m-%d %H:%M:%S")

    # 🔥 GLOBAL RESET (EVERY CYCLE)
    ...
    for tf_index, tf_label in TF_INDEX_MAP.items():
        ...
        try:
            ...
            i = len(df) - 1
            live_price = float(df["close"].iloc[i])

            debug_log(
                f"[DF READY] {stock_cfg.get('stock_code')} {tf_label} "
                f"close={live_price:.2f}"
            )
            ...
            debug_log(
                f"[EXEC CHECK] {stock_cfg.get('stock_code')} {tf_label} "
                f"ALGO={resolved['algo']} "
                f"mode={exec_mode} "
                f"entry={resolved['entry']} live={live_price} target={resolved['target']}"
            )

            # ❌ DO NOT ALERT IF NOT EXECUTABLE
            if exec_mode in ("WAIT", "INVALID"):
                debug_log(
                    f"[SKIP ALERT] {stock_cfg.get('stock_code')} {tf_label} "
                    f"reason={exec_mode}"
                )
                continue

            # ----------------------------------
            # ✅ BUY CONFIRMED (STOP / MARKET)
            # ----------------------------------
            resolved["execution"] = exec_mode
            ...
            debug_log(
                f"[BUY CONFIRMED] {stock_cfg.get('stock_code')} {tf_label} "
                f"ALGO={resolved['algo']} MODE={exec_mode}"
            )
