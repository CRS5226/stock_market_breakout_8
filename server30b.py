...
# from basic_algo5 import basic_forecast_update
...
def _process_stock_forecast(stock_cfg, use_llm=False, verbose=True):
    """
    Runs forecast for a single stock (called inside threadpool).
    Returns tuple: (stock_code, updated_cfg (dict), list_of_buy_signals)
    """
    stock_code = stock_cfg.get("stock_code")
    updated_cfg = None
    buy_signals = []  # list of (tf_label, payload) that should trigger Telegram
    ...
    try:
        ...
        if use_llm:
            try:
                ...
                if verbose:
                    print(f"[LLM ERROR] {stock_code}: {e}")
        ...
        try:
            ...
            if verbose:
                print(f"[⚠️ Redis Save Forecast] {stock_code}: {e_redis}")

        # --- Prepare BUY signals for 5min / 30min / 1hour ---
        TF_INDEX_MAP = {
            0: "1min",
            1: "5min",
            2: "15min",
            ...
            5: "1hour",
            6: "4hour",
            7: "1day",  # ✅ Include daily BUY alerts
            8: "1month",  # ✅ Include monthly BUY alerts
        }

        for idx, tf_label in TF_INDEX_MAP.items():
            sig_key = "signal" if idx == 0 else f"signal{idx}"
            sig_val = updated_cfg.get(sig_key)
            ...
            # Extract ALGO name from updated_cfg (saved by basic_algo5)
            algo_map = updated_cfg.get("algo_signals", {})
            algo_list = algo_map.get(tf_label, [])
            algo_name = algo_list[0] if algo_list else "UNKNOWN"

            payload = {
                "ALGO": algo_name,  # ✅ Add ALGO reason
                "stock_code": stock_code,
                "timeframe": tf_label,
                "tf_index": idx,
                "signal": "BUY",
            ...
            # --- Save BUY signal status to Redis ---
            try:
                r = get_redis()
                should_alert = save_buy_signal_to_redis(
                    r, stock_code, tf_label, payload
                )

                # Debug trace
                print(f"[DEBUG] {stock_code} {tf_label}: should_alert={should_alert}")

                # Only append for alert if Redis says it's new
                if should_alert:
                    buy_signals.append((tf_label, payload))

            except Exception as e_bs:
                if verbose:
                    print(f"[⚠️ Redis BUY save failed] {stock_code} {tf_label}: {e_bs}")
